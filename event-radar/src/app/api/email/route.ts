...
const APP_URL = process.env.NEXT_PUBLIC_APP_URL || 'https://events-for-christian.vercel.app';
...
export async function GET(request: NextRequest) {
  ...
  const { data: introLog } = await supabase
    .from('email_log')
    .select('id')
    .eq('email_type', 'intro')
    .limit(1);
...
async function sendDailyDigest() {
  ...
  const sinceDate = lastEmail?.[0]?.sent_at || new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
